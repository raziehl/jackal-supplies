...
export class NatsRouter {
  ...
  constructor(options: NatsRouterOptions) {
    ...
    process.on('SIGTERM', async () => {
      await this.drain();
      log.debug("[NATS] finished draining and closed the connections, exiting process now");
      process.exit(1);
    });
    ...
    this.nats.subscribe(
      ...
      async (payload: string, replyTo: string) => {
        ...
        try {
          ...
          log.error(`[NatsRouter.subscribeQuery] Failed to parse payload for subject ${subject}`);
        }
        try {
          const str = JSON.stringify(result);
          this.nats.publish(replyTo, str);
        } catch (e) {
          log.error(`[NatsRouter.subscribeQuery] Failed to stringify response for subject ${subject}`)
        }
      }
    );
  }
  ...
  public drain(): Promise<boolean> {
    return new Promise((resolve, reject) => {
      this.nats.drain((error: Error) => {
        this.disconnect();
        log.info("[NATS] Drained all NATS connection", error);
        if (error) {
          log.debug("[NATS] Error thrown when draining nats connection", error);
          reject(false);
        } else {
          resolve(true)
        }
  ...
  private setupConnectionEvents(): void {
    ...
    this.stan.on('connect', () => {
      log.debug('[STAN] connected');
      log.debug(`Connected to stan - ${this.clientId}`);
